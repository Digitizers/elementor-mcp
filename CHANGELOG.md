...
- **Admin: unlock takes effect.** The Tools tab renders the SEO/A11y + Widget Builder categories for any install with the pack enabled (was gated behind the dead license check, leaving the tools invisible/unremovable), and a v4 defaults migration removes those slugs from `elementor_mcp_disabled_tools` so previously-seeded installs get them **enabled** rather than silently filtered out.
