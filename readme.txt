...
* Verified live on WP 6.9 / Elementor 4.2.2 / Angie 1.1.11: Angie discovers the six read-only tools and executes them. Note: the tool toggles on the admin Tools tab filter abilities at registration, so a tool switched off there is absent from the bridge too — check that list-widgets, get-widget-schema, get-page-structure and list-pages are enabled if the bridge lists nothing. Low-tools mode keeps those four (they are essentials) but does drop list-global-classes and list-variables.
