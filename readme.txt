...
* Verified live on WP 6.9 / Elementor 4.2.2 / Angie 1.1.11: Angie discovers the six read-only tools and executes them. Note: the tool toggles on the admin Tools tab filter abilities at registration, so a disabled tool is absent from the bridge too — the allowlist needs list-widgets, get-widget-schema, get-page-structure and list-pages enabled (low-tools mode disables all four).
