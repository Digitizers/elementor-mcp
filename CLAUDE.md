...
**Declared intent on page writes (1.36.0).** `Elementor_MCP_Collateral::report()` DERIVES a write's targets by diffing the stored tree against the requested one, so anything the tool itself changed is a target by construction — an ability that rewrites an unrelated node is invisible to every check in the write path. A write may therefore also declare what it set out to touch: `save_page_data( $post_id, $data, array( 'scope' => 'targeted', 'ids' => array( $element_id ) ) )` or `array( 'scope' => 'document' )`. The report then carries `compared_by` (always `'id'` — nodes are matched by id and only by id, and every id it names is a string), `intent`, and `undeclared`. What `undeclared` judges: the derived targets **plus nodes that merely moved** (present in both trees under a different parent — a move changes no payload, so it is never a target). A node present in both trees is covered only when **both** ancestries place it inside the declaration — either alone lets a relocation launder the coverage; a node only in one tree is judged by that tree. Coverage is computed from each node's own path as the tree is walked (never by climbing an id → parent map), so it is O(n) — one scan per tree, plus a second scan of the requested tree only when the write introduced a duplicate — and a duplicated id neither adopts an unrelated subtree nor shuts a legitimate one out. Duplicate ids split by origin: one **inherited** (already ambiguous in the before tree) is pre-existing page corruption, so the duplicated node itself is never reported — that is the one thing here that stays invisible, deliberately, since `index()` declines to compare its payload for the same reason; one **introduced** by this write (unique or absent before, ambiguous after) confers no coverage on anything under any of its occurrences and is itself reported, so a write cannot mint a copy of the declared id to smuggle a subtree in, nor plant a decoy sharing an unrelated node's id to buy it an exemption. A same-parent sibling reorder is invisible and the class docblock says so. Declaring is optional and additive — passing nothing is exactly the 1.34.0 behaviour, and a malformed declaration reads as undeclared and never fails a write. `undeclared` is **warn-only** in this release: it rides the run's existing `warnings` channel as `{ rule: 'collateral', reason }` in every `emcp_collateral_guard_mode` mode but `off`, and reverts under none of them. It fires its own action, `elementor_mcp_governance_undeclared( $name, $post_id, $report, $mode, $reverted )` — `$reverted` because the same run may have carried real collateral under `refuse` and been rolled back for that. `elementor_mcp_governance_collateral` still fires only for what the SAVE did, unchanged since 1.34.0; both fire when both are true. Note that `emcp_collateral_guard_mode` is now consulted on undeclared-only runs, which never reached it before 1.36.0. Declared so far: `update-element` / `update-widget` / `update-container` / `remove-element` (targeted) and `delete-page-content` / `import-template` / `build-page` (document). Declare only what an ability can state truthfully — no heuristics that guess intent.
