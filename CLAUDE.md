...
**Declared intent on page writes (1.36.0).** `Elementor_MCP_Collateral::report()` DERIVES a write's targets by diffing the stored tree against the requested one, so anything the tool itself changed is a target by construction — an ability that rewrites an unrelated node is invisible to every check in the write path. A write may therefore also declare what it set out to touch: `save_page_data( $post_id, $data, array( 'scope' => 'targeted', 'ids' => array( $element_id ) ) )` or `array( 'scope' => 'document' )`. The report then carries `compared_by` (always `'id'` — nodes are matched by id and only by id), `intent`, and `undeclared` (derived targets outside the declaration; a descendant of a declared id, in either tree, counts as covered). Declaring is optional and additive — passing nothing is exactly the 1.34.0 behaviour, and a malformed declaration reads as undeclared and never fails a write. `undeclared` is **warn-only** in this release: it rides the run's existing `warnings` channel as `{ rule: 'collateral', reason }` in every `emcp_collateral_guard_mode` mode but `off`, and reverts under none of them. Declared so far: `update-element` / `update-widget` / `update-container` / `remove-element` (targeted) and `delete-page-content` / `import-template` / `build-page` (document). Declare only what an ability can state truthfully — no heuristics that guess intent.
