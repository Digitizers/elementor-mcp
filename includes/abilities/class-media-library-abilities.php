 ...
 *
 * Ported from upstream msrbuilds/elementor-mcp (#25, v2.0.2), adapted to this
 * fork's class/helper naming (the upstream rename to emcp-tools is not adopted).
 *
 ...
 */
class Elementor_MCP_Media_Library_Abilities {
			...
			array(
				...
				'input_schema'        => array(
					...
					'properties' => array(
						'search'    => array(
							'type'        => 'string',
							'description' => __( 'Keyword to match against the attachment title, alt text, caption, and description. Omit to list everything.', 'elementor-mcp' ),
						),
						'mime_type' => array(
							'type'        => 'string',
							'description' => __( 'MIME type filter. Accepts a top-level type ("image") or a specific type ("image/jpeg", "image/png"). Use "any" for all media types. Default: image.', 'elementor-mcp' ),
						),
						'page'      => array(
							'type'        => 'integer',
							'description' => __( 'Page number (1-based). Default: 1.', 'elementor-mcp' ),
						),
						'per_page'  => array(
							'type'        => 'integer',
							'description' => __( 'Results per page (1-100). Default: 20.', 'elementor-mcp' ),
						),
						'orderby'   => array(
							'type'        => 'string',
							'enum'        => array( 'date', 'title' ),
							'description' => __( 'Sort field. Default: date (newest first).', 'elementor-mcp' ),
						),
						'order'     => array(
							'type'        => 'string',
							'enum'        => array( 'desc', 'asc' ),
							'description' => __( 'Sort direction. Default: desc.', 'elementor-mcp' ),
						),
					),
				),
				'output_schema'       => array(
