...
- New: **Server-enforced approval grants** for governed Elementor page writes — the second plank of P0.2. When SiteAgent's Ed25519 grant regime is active (a gateway key is provisioned) **and** grant enforcement is opted in for this plugin, a governed page write must present a valid `X-Aura-Approval-Grant` bound to its exact tool + params before it may persist. The grant is verified at the **page-write site** (before the pre-write snapshot) via SiteAgent's `Aura_Worker_Grant::verify()`, which checks the signature, tool/params/site binding, validity window, and single-use nonce. A missing grant returns `governance_grant_required`; a rejected one returns `governance_grant_invalid` — in both cases the write never snapshots or persists.
  - **Checked at the write site, so previews are exempt.** Like the snapshot, the grant is enforced only when a tool actually writes page data — so an a11y/SEO generator run as a preview (`apply=false`), or any call that writes no page data, needs no grant. The grant binds to the **exposed MCP tool name** (`/` → `-`, e.g. `elementor-mcp-update-element`), matching what the gateway signs.
